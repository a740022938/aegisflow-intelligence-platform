...
Assistant Center 是 AIP v7.3.0 的只读助手作战中枢，用于在一个页面内查看 AIP、OpenAxiom、OpenClaw、Claude Proxy、运行工具、报告、备份和高风险边界。
...
v7.3.0 默认只读。所有 API 返回 `readonly: true`，涉及状态项时返回 `autoFixAllowed: false`。
...
`audit/save` 在 v7.3.0 返回 disabled，不自动写入审计日志。
