...
const AuditInterceptor_js_1 = require("./AuditInterceptor.js");
const DbAuditLogger_js_1 = require("./DbAuditLogger.js");
const DbAdapter_js_1 = require("./DbAdapter.js");
/**
...
class PluginManager {
    ...
    constructor(options = {}) {
        ...
        // 初始化加载器
        this.loader = new Loader_js_1.PluginLoader(this.options.pluginDir || './plugins/builtin');
        // 初始化审计拦截器
        // Phase 0.5: 接入 DbAuditLogger
        const dbAdapter = (0, DbAdapter_js_1.getDbAdapter)();
        const auditOptions = {
            dbAudit: dbAdapter ? new DbAuditLogger_js_1.DbAuditLogger({
                db: dbAdapter,
        ...
        this.audit = new AuditInterceptor_js_1.AuditInterceptor(auditOptions);
        // 如果启用，自动加载内置插件
    ...
    async registerPlugin(manifest) {
        ...
        // 审计
        await this.audit.onPluginRegistered(manifest.plugin_id, manifest.name, manifest.risk_level);
    ...
    validatePluginManifest(manifest) {
        return (0, plugin_sdk_1.validateManifest)(manifest);
    }
    /**
     * 启用插件
     */
    ...
    listPlugins(options) {
        ...
        if (options?.search) {
            ...
            plugins = plugins.filter((p) => p.manifest.name.toLowerCase().includes(search) ||
        ...
        return plugins.map((p) => ({
            ...
            category: p.manifest.category || 'legacy/unknown',
            status: p.manifest.status || 'active',
            execution_mode: p.manifest.execution_mode || 'readonly',
            requires_approval: p.manifest.requires_approval ?? false,
            dry_run_supported: p.manifest.dry_run_supported ?? false,
            ui_node_type: p.manifest.ui_node_type,
            allowed_upstream: p.manifest.allowed_upstream,
            allowed_downstream: p.manifest.allowed_downstream,
            input_schema: p.manifest.input_schema,
            output_schema: p.manifest.output_schema,
            icon: p.manifest.icon,
            color: p.manifest.color,
            documentation_url: p.manifest.documentation_url,
        }));
    }
    /**
     * 加载内置插件
    ...
    async loadBuiltinPlugins() {
        ...
        for (const plugin of loaded) {
            ...
            if (!this.plugins.has(plugin.manifest.plugin_id)) {
                ...
                await this.audit.onPluginRegistered(plugin.manifest.plugin_id, plugin.manifest.name, plugin.manifest.risk_level);
            }
        }
    }
    // ===================================================================
     ...
     * └────────────────┴────────┴──────────────┴──────────┴─────────────┘
     */
    checkPluginExecutionGate(pluginId, options) {
        // 系统未启用
        if (!this.systemEnabled) {
            return {
                allowed: false,
        ...
        // ===== 生命周期门禁 =====
        const status = plugin.manifest.status || 'active';
        const executionMode = plugin.manifest.execution_mode || 'readonly';
     ...
     * 执行插件（带强制门禁）
     */
    async executePlugin(pluginId, action, params) {
        const startTime = Date.now();
        const dryRun = params?.dry_run === true;
        const rawExecutionMode = params?.execution_mode ||
            (this.plugins.get(pluginId)?.manifest?.execution_mode || 'readonly');
        const executionMode = rawExecutionMode;
        // ===== 前置：写入 execute_attempt 审计（不阻断）=====
        await this.audit.onPluginExecuted(pluginId, action, params, { success: false, _stub: true }, 0, executionMode, dryRun).catch(() => { });
        // ===== 执行门禁检查 =====
        const gate = this.checkPluginExecutionGate(pluginId, { dry_run: dryRun });
        if (!gate.allowed) {
            // 写入 execute_blocked 审计
            await this.audit.onPluginExecutionBlocked(pluginId, action, gate.reason || 'Execution gate denied', executionMode).catch(() => { });
            return {
                success: false,
                plugin_id: pluginId,
                action,
        ...
        try {
            ...
            // 审计：execute_success
            await this.audit.onPluginExecuted(pluginId, action, params, result, durationMs, executionMode, dryRun);
            return {
                success: true,
                plugin_id: pluginId,
                action,
