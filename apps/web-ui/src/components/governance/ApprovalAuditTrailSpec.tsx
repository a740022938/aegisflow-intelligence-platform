...
import { APPROVAL_AUDIT_TRAIL_FIELDS } from './governanceDesignSpec';

export default function ApprovalAuditTrailSpec() {
  return (
    <div>
      <div style={{ marginBottom: 8, fontSize: 11, color: 'var(--text-muted)', lineHeight: 1.6 }}>
        Approval audit trail spec — all fields are <strong>not persisted / design-only / no write path / Stage C deferred</strong>.
      </div>
      <div style={{ display: 'grid', gap: 4, fontSize: 10 }}>
        {APPROVAL_AUDIT_TRAIL_FIELDS.map(f => (
      ...
      <div style={{ marginTop: 6, padding: '4px 8px', borderRadius: 4, background: 'rgba(139,92,246,0.04)', fontSize: 9, color: 'var(--text-muted)', fontStyle: 'italic' }}>
        Audit trail design specification — not persisted. No DB schema, no API endpoint, no write path. Stage C deferred.
      </div>
    </div>
  );
}
