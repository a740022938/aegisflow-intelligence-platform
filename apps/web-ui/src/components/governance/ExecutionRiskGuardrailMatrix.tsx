...
import { EXECUTION_GUARDRAIL_MATRIX } from './governanceDesignSpec';
...
export default function ExecutionRiskGuardrailMatrix() {
  return (
    <div>
      <div style={{ marginBottom: 8, fontSize: 11, color: 'var(--text-muted)', lineHeight: 1.6 }}>
        Execution risk guardrail matrix — all risks are currently <strong>safe</strong> with <strong>no active exposure</strong>. No execution action, service control, deployment, lab execution, training, inference, external write, or tag/release is possible.
      </div>
      <div style={{ display: 'grid', gap: 2, fontSize: 10 }}>
        ...
        {EXECUTION_GUARDRAIL_MATRIX.map(r => (
