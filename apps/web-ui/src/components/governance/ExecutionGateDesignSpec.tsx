...
import { EXECUTION_DESIGN_FIELDS, EXECUTION_EVIDENCE_TYPES, EXECUTION_LIFECYCLE_STAGES } from './governanceDesignSpec';
...
export default function ExecutionGateDesignSpec() {
  return (
    <div>
      <div style={{ padding: '8px 12px', marginBottom: 10, borderRadius: 4, background: 'rgba(139,92,246,0.08)', border: '1px solid rgba(139,92,246,0.2)', fontSize: 11, color: 'var(--text-secondary)', lineHeight: 1.6 }}>
        <strong>Execution Gate is design-only.</strong> Stage C is not enabled. No execution, run, start, stop, deployment, sync, service-control or mutation action is available.
      </div>

      <div style={{ display: 'grid', gap: 8, marginBottom: 16 }}>
        {EXECUTION_DESIGN_FIELDS.map(f => (
          <div key={f.fieldName} style={{ padding: '10px 14px', borderRadius: 6, background: 'var(--bg-surface)', border: '1px solid var(--border)', borderLeft: '3px solid #8B5CF6', fontSize: 11 }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', fontSize: 10, color: 'var(--text-muted)' }}>
              ...
              <span>Execute: <strong style={{ color: 'var(--success)' }}>{f.executePermission}</strong></span>
              <span>Write: <strong style={{ color: 'var(--success)' }}>{f.writePath}</strong></span>
              <span>Blocked: {f.blockedActions}</span>
      ...
      <div style={{ fontSize: 12, fontWeight: 600, color: '#8B5CF6', marginBottom: 8 }}>Execution Evidence Types</div>
      <div style={{ display: 'grid', gap: 4, marginBottom: 16 }}>
        {EXECUTION_EVIDENCE_TYPES.map(e => (
      ...
      <div style={{ fontSize: 12, fontWeight: 600, color: '#8B5CF6', marginBottom: 8 }}>Execution Lifecycle Stages</div>
      <div style={{ display: 'grid', gap: 4, marginBottom: 8 }}>
        {EXECUTION_LIFECYCLE_STAGES.map(s => (
      ...
      <div style={{ marginTop: 8, padding: '6px 10px', borderRadius: 4, background: 'rgba(245,158,11,0.06)', fontSize: 10, color: 'var(--text-muted)', fontStyle: 'italic' }}>
        Execution Gate design specification — no runtime controls enabled. All fields are design-only. Evidence and lifecycle stages are speculative.
      </div>
    </div>
  );
}
