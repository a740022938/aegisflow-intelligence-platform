...
import { AUTHORIZATION_LIFECYCLE_STAGES } from './governanceDesignSpec';

export default function AuthorizationRequestLifecycle() {
  return (
    <div style={{ fontSize: 10 }}>
      <div style={{ marginBottom: 8, padding: '6px 10px', borderRadius: 4, background: 'rgba(139,92,246,0.06)', border: '1px solid rgba(139,92,246,0.2)', color: 'var(--text-secondary)' }}>
        Authorization Request Lifecycle Design — <strong>{AUTHORIZATION_LIFECYCLE_STAGES.length}</strong> stages. All stages are <strong>design-only</strong>. No runtime effect, no persistence. Stage C deferred.
      </div>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 10 }}>
        ...
        <tbody>
          {AUTHORIZATION_LIFECYCLE_STAGES.map(s => (
