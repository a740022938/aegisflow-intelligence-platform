...
import { DEPLOYMENT_ROLLBACK_LIFECYCLE_STAGES } from './governanceDesignSpec';

export default function DeploymentRollbackLifecycleDesign() {
  return (
    <div style={{ fontSize: 10 }}>
      <div style={{ marginBottom: 8, padding: '6px 10px', borderRadius: 4, background: 'rgba(139,92,246,0.06)', border: '1px solid rgba(139,92,246,0.2)', fontSize: 10, color: 'var(--text-secondary)' }}>
        Deployment / Rollback Lifecycle Design — all stages are <strong>design-only</strong> with <strong>no runtime effect</strong>. Stage C deferred. No state machine, API, or DB writes are active.
      </div>

      <div style={{ marginBottom: 6, fontSize: 11, fontWeight: 600, color: 'var(--text-primary)' }}>
        Deployment / Rollback Lifecycle Stages ({DEPLOYMENT_ROLLBACK_LIFECYCLE_STAGES.length})
      </div>

      <div style={{ display: 'grid', gap: 2, fontSize: 9, overflowX: 'auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1.8fr 2.5fr 1.8fr', gap: 8, padding: '5px 8px', color: 'var(--text-muted)', fontWeight: 600, borderBottom: '1px solid var(--border)' }}>
          <span>Stage</span><span>Purpose</span><span>Status</span>
        </div>
        {DEPLOYMENT_ROLLBACK_LIFECYCLE_STAGES.map(s => (
          <div key={s.stage} style={{
            display: 'grid', gridTemplateColumns: '1.8fr 2.5fr 1.8fr', gap: 8,
