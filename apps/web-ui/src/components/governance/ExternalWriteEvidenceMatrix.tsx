...
import { EXTERNAL_WRITE_EVIDENCE_TYPES } from './governanceDesignSpec';

export default function ExternalWriteEvidenceMatrix() {
  return (
    <div style={{ fontSize: 10 }}>
      <div style={{ marginBottom: 8, padding: '6px 10px', borderRadius: 4, background: 'rgba(139,92,246,0.06)', border: '1px solid rgba(139,92,246,0.2)', fontSize: 10, color: 'var(--text-secondary)' }}>
        External Write Evidence Matrix — all evidence types are <strong>readonly / design-only</strong>. No runtime effect. No real evidence is collected or persisted.
      </div>

      <div style={{ marginBottom: 6, fontSize: 11, fontWeight: 600, color: 'var(--text-primary)' }}>
        External Write Evidence Types ({EXTERNAL_WRITE_EVIDENCE_TYPES.length})
      </div>

      <div style={{ display: 'grid', gap: 2, fontSize: 9, overflowX: 'auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 2fr 1.5fr', gap: 8, padding: '5px 8px', color: 'var(--text-muted)', fontWeight: 600, borderBottom: '1px solid var(--border)' }}>
          <span>Evidence</span><span>Purpose</span><span>Status</span>
        </div>
        {EXTERNAL_WRITE_EVIDENCE_TYPES.map(e => (
          <div key={e.evidence} style={{
            display: 'grid', gridTemplateColumns: '1.5fr 2fr 1.5fr', gap: 8,
      ...
      <div style={{ marginTop: 8, padding: '6px 10px', borderRadius: 4, background: 'rgba(139,92,246,0.04)', fontSize: 9, color: 'var(--text-muted)', lineHeight: 1.6 }}>
        <strong>约束：</strong>不读取真实外部端点。不上传 payload。不写 DB。不新增真实状态机。所有 evidence 类型均为设计规格参考。
      </div>
    </div>
  );
}
