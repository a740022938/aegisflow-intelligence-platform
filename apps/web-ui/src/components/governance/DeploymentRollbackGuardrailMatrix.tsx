...
import { DEPLOYMENT_ROLLBACK_GUARDRAIL_MATRIX } from './governanceDesignSpec';

export default function DeploymentRollbackGuardrailMatrix() {
  return (
    <div style={{ fontSize: 10 }}>
      <div style={{ marginBottom: 8, padding: '6px 10px', borderRadius: 4, background: 'rgba(139,92,246,0.06)', border: '1px solid rgba(139,92,246,0.2)', fontSize: 10, color: 'var(--text-secondary)' }}>
        Deployment / Rollback Guardrail Matrix — all active risks are <strong>0</strong>. No deploy, release, tag, push, upload, rollback, restore, or sync paths are active. All statuses are <strong>safe</strong>.
      </div>
      ...
      <div style={{ display: 'grid', gap: 2, fontSize: 9, overflowX: 'auto' }}>
        ...
        {DEPLOYMENT_ROLLBACK_GUARDRAIL_MATRIX.map(r => (
