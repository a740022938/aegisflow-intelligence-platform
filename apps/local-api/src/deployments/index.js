...
exports.createDeploymentFromArtifact = createDeploymentFromArtifact;
exports.startDeployment = startDeployment;
exports.stopDeployment = stopDeployment;
exports.restartDeployment = restartDeployment;
...
const createDeploymentSchema = zod_1.z.object({
    ...
    deployment_type: zod_1.z.enum(['local_api', 'model_service', 'batch_worker', 'evaluation_runner', 'custom']).default('local_api'),
    runtime: zod_1.z.enum(['mock', 'python', 'fastapi', 'node', 'custom']).default('mock'),
    status: zod_1.z.enum(['created', 'deploying', 'running', 'stopped', 'failed', 'archived', 'deleted']).default('created'),
    ...
    entrypoint: zod_1.z.string().default(''),
    model_path: zod_1.z.string().default(''),
    config_json: zod_1.z.string().default('{}'),
    health_status: zod_1.z.enum(['unknown', 'healthy', 'unhealthy', 'starting', 'stopped']).default('unknown'),
    notes: zod_1.z.string().default(''),
});
const updateDeploymentSchema = zod_1.z.object({
    name: zod_1.z.string().optional(),
    deployment_type: zod_1.z.enum(['local_api', 'model_service', 'batch_worker', 'evaluation_runner', 'custom']).optional(),
    runtime: zod_1.z.enum(['mock', 'python', 'fastapi', 'node', 'custom']).optional(),
    status: zod_1.z.enum(['created', 'deploying', 'running', 'stopped', 'failed', 'archived', 'deleted']).optional(),
    ...
    health_status: zod_1.z.enum(['unknown', 'healthy', 'unhealthy', 'starting', 'stopped']).optional(),
...
async function listDeployments(query = {}) {
    ...
    return {
        ok: true,
        deployments: rows.map((r) => ({ ...r, config_json: parseJsonField(r.config_json) })),
...
async function getDeploymentById(id) {
    ...
    if (!row)
        return { ok: false, error: 'Deployment ' + id + ' not found' };
...
async function createDeployment(data) {
  ...
  ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(id, d.name, d.deployment_type, d.runtime, d.status, d.artifact_id, d.artifact_name, d.training_job_id, d.evaluation_id, d.host, port, baseUrl, d.entrypoint, d.model_path, d.config_json, d.health_status, d.notes, n, n);
...
async function createDeploymentFromArtifact(artifactId, extraData = {}) {
    const db = (0, builtin_sqlite_js_1.getDatabase)();
  ...
  ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(id, 'Deploy: ' + artifact.name, deploymentType, runtime, 'created', artifactId, artifact.name, artifact.training_job_id || '', host, port, 'http://' + host + ':' + port, artifact.model_path || artifact.path || '', '{}', 'unknown', 'Auto-created from artifact ' + artifactId, n, n);
...
async function startDeployment(id) {
    ...
    if (row.status === 'deleted')
        return { ok: false, error: 'Cannot start a deleted deployment' };
    const n = now();
...
async function stopDeployment(id) {
    ...
    if (row.status === 'stopped')
        return { ok: false, error: 'Deployment already stopped' };
    if (row.status === 'deleted')
        return { ok: false, error: 'Cannot stop a deleted deployment' };
...
async function restartDeployment(id) {
    ...
    if (!row)
        return { ok: false, error: 'Deployment ' + id + ' not found' };
    if (row.status === 'deleted')
        return { ok: false, error: 'Cannot restart a deleted deployment' };
...
// ── Logs ─────────────────────────────────────────────────────────────────────
async function getDeploymentLogs(deploymentId, query = {}) {
    const db = (0, builtin_sqlite_js_1.getDatabase)();
...
// ── Health ───────────────────────────────────────────────────────────────────
async function getDeploymentHealth(id) {
    const db = (0, builtin_sqlite_js_1.getDatabase)();
    const row = db.prepare('SELECT * FROM deployments WHERE id = ?').get(id);
    if (!row)
        return { ok: false, error: 'Deployment ' + id + ' not found' };
