...
exports.listPipelineConfigs = listPipelineConfigs;
exports.getPipelineConfig = getPipelineConfig;
exports.createPipelineConfig = createPipelineConfig;
exports.createPipelineRun = createPipelineRun;
exports.listPipelineRuns = listPipelineRuns;
exports.getPipelineRun = getPipelineRun;
exports.getPipelineRunByRunId = getPipelineRunByRunId;
exports.createSplit = createSplit;
exports.listSplits = listSplits;
exports.linkDatasetToRun = linkDatasetToRun;
...
// ── Schemas ──────────────────────────────────────────────────────────────────
const createPipelineConfigSchema = zod_1.z.object({
    config_code: zod_1.z.string().min(1, 'config_code is required'),
    name: zod_1.z.string().min(1, 'name is required'),
    description: zod_1.z.string().default(''),
    pipeline_type: zod_1.z.enum(['import', 'clean', 'split', 'augment', 'validate', 'full']).default('full'),
    steps_json: zod_1.z.union([zod_1.z.string(), zod_1.z.array(zod_1.z.string())]).default([]),
    default_params_json: zod_1.z.union([zod_1.z.string(), zod_1.z.any()]).default({}),
    env_vars_json: zod_1.z.union([zod_1.z.string(), zod_1.z.any()]).default({}),
});
const createPipelineRunSchema = zod_1.z.object({
    name: zod_1.z.string().min(1, 'name is required'),
    dataset_id: zod_1.z.string().min(1, 'dataset_id is required'),
    pipeline_config_id: zod_1.z.string().optional(),
    pipeline_type: zod_1.z.enum(['import', 'clean', 'split', 'augment', 'validate', 'full']).default('full'),
    config_json: zod_1.z.union([zod_1.z.string(), zod_1.z.any()]).default({}),
});
const createSplitSchema = zod_1.z.object({
    dataset_pipeline_run_id: zod_1.z.string().min(1, 'dataset_pipeline_run_id is required'),
    dataset_id: zod_1.z.string().min(1, 'dataset_id is required'),
    split_name: zod_1.z.string().min(1, 'split_name is required'),
    sample_count: zod_1.z.number().int().min(0).default(0),
    file_path: zod_1.z.string().default(''),
    record_count: zod_1.z.number().int().min(0).default(0),
    checksum: zod_1.z.string().default(''),
    config_json: zod_1.z.union([zod_1.z.string(), zod_1.z.any()]).default({}),
});
...
function listPipelineConfigs(query = {}) {
...
function getPipelineConfig(id) {
    const db = (0, builtin_sqlite_js_1.getDatabase)();
    const row = db.prepare('SELECT * FROM dataset_pipeline_configs WHERE id = ?').get(id);
    if (!row)
...
function createPipelineConfig(body) {
    const db = (0, builtin_sqlite_js_1.getDatabase)();
    ...
    try {
      ...
      VALUES (?,?,?,?,?,?,?,?,?,?,?)
    `).run(id, d.config_code, d.name, d.description, d.pipeline_type, stepsStr, paramsStr, envStr, 0, n, n);
    }
    catch (e) {
        if (e.message.includes('UNIQUE'))
            return { ok: false, error: `Config with code "${d.config_code}" already exists` };
...
function createPipelineRun(body) {
    const db = (0, builtin_sqlite_js_1.getDatabase)();
    ...
    const ds = db.prepare('SELECT id FROM datasets WHERE id = ?').get(d.dataset_id);
    if (!ds)
    ...
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
  `).run(runId, code, d.name, 'dataset', d.dataset_id, 'running', 5, 'manual', 'mock', '', configStr, '{}', '', n, n);
    ...
    VALUES (?,?,?,?,?,?,?,?,?,?)
  `).run(pipelineRunId, runId, d.dataset_id, d.pipeline_config_id || '', d.pipeline_type, 'running', configStr, n, n, n);
    ...
    void _executePipelineRun(pipelineRunId, runId, d.dataset_id, d.pipeline_type).catch((err) => {
        const db2 = (0, builtin_sqlite_js_1.getDatabase)();
        const nn = now();
        db2.prepare('UPDATE dataset_pipeline_runs SET status=?, finished_at=?, error_message=?, updated_at=? WHERE id=?')
    ...
    const pipelineRun = db.prepare('SELECT * FROM dataset_pipeline_runs WHERE id = ?').get(pipelineRunId);
    return { ok: true, run, pipeline_run: pipelineRun };
}
async function _executePipelineRun(pipelineRunId, runId, datasetId, pipelineType) {
    const db = (0, builtin_sqlite_js_1.getDatabase)();
    const defs = PIPELINE_STEP_DEFS[pipelineType] || PIPELINE_STEP_DEFS.full;
    const startTime = now();
    const step0Id = generateId();
    db.prepare('INSERT INTO run_steps (id,run_id,step_key,step_name,step_order,status,started_at,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?)')
    ...
    for (let i = 0; i < defs.length; i++) {
        ...
        db.prepare('UPDATE run_steps SET status=?, finished_at=?, duration_ms=?, output_json=?, updated_at=? WHERE id=?')
        ...
        if ((pipelineType === 'split' || pipelineType === 'full') && def.key === 'split_data') {
            ...
            for (const s of splits) {
                ...
                db.prepare('INSERT INTO dataset_splits (id,dataset_pipeline_run_id,dataset_id,split_name,sample_count,file_path,record_count,config_json,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)')
    ...
    db.prepare('UPDATE dataset_pipeline_runs SET status=?, output_sample_count=?, started_at=?, finished_at=?, updated_at=? WHERE id=?')
        .run('success', outputSamples, startTime, finishTime, now(), pipelineRunId);
    db.prepare('UPDATE runs SET status=?, started_at=?, finished_at=?, summary_json=?, updated_at=? WHERE id=?')
...
// ── Pipeline Run CRUD ──────────────────────────────────────────────────────────
function listPipelineRuns(query = {}) {
    const db = (0, builtin_sqlite_js_1.getDatabase)();
...
function getPipelineRun(id) {
    const db = (0, builtin_sqlite_js_1.getDatabase)();
    const row = db.prepare('SELECT * FROM dataset_pipeline_runs WHERE id = ?').get(id);
    if (!row)
        return { ok: false, error: 'Pipeline run not found' };
    const run = db.prepare('SELECT * FROM runs WHERE id = ?').get(row.run_id);
    const splits = db.prepare('SELECT * FROM dataset_splits WHERE dataset_pipeline_run_id = ? ORDER BY split_name').all(id);
    return {
        ok: true,
...
function getPipelineRunByRunId(runId) {
    const db = (0, builtin_sqlite_js_1.getDatabase)();
    const row = db.prepare('SELECT * FROM dataset_pipeline_runs WHERE run_id = ?').get(runId);
    if (!row)
        return { ok: false, error: 'Pipeline run not found' };
...
function createSplit(body) {
    ...
    if (!validation.success)
        return { ok: false, error: validation.error.errors[0].message };
    const d = validation.data;
    const pr = db.prepare('SELECT id FROM dataset_pipeline_runs WHERE id = ?').get(d.dataset_pipeline_run_id);
    if (!pr)
        return { ok: false, error: 'Pipeline run not found' };
    const configStr = typeof d.config_json === 'string' ? d.config_json : JSON.stringify(d.config_json || {});
    const id = generateId();
    const n = now();
    db.prepare('INSERT INTO dataset_splits (id,dataset_pipeline_run_id,dataset_id,split_name,sample_count,file_path,record_count,checksum,config_json,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)')
    ...
    const total = db.prepare('SELECT SUM(sample_count) as total FROM dataset_splits WHERE dataset_pipeline_run_id = ?').get(d.dataset_pipeline_run_id);
    db.prepare('UPDATE dataset_pipeline_runs SET output_sample_count=?, updated_at=? WHERE id=?')
        .run(total?.total || d.sample_count, n, d.dataset_pipeline_run_id);
    const row = db.prepare('SELECT * FROM dataset_splits WHERE id = ?').get(id);
    return { ok: true, split: row };
}
function listSplits(query = {}) {
    const db = (0, builtin_sqlite_js_1.getDatabase)();
    const { dataset_id, dataset_pipeline_run_id } = query;
    let sql = 'SELECT * FROM dataset_splits WHERE 1=1';
    const params = [];
    if (dataset_pipeline_run_id) {
        sql += ' AND dataset_pipeline_run_id = ?';
        params.push(dataset_pipeline_run_id);
    }
    if (dataset_id) {
        sql += ' AND dataset_id = ?';
        params.push(dataset_id);
...
function linkDatasetToRun(datasetId, runId, relationType = 'input') {
    const db = (0, builtin_sqlite_js_1.getDatabase)();
    if (!(db.prepare('SELECT id FROM runs WHERE id = ?').get(runId)))
...
function getPipelineSummary() {
    const db = (0, builtin_sqlite_js_1.getDatabase)();
    const all = db.prepare('SELECT status, COUNT(*) as count FROM dataset_pipeline_runs GROUP BY status').all();
    const total = db.prepare('SELECT COUNT(*) as total FROM dataset_pipeline_runs').get().total;
    const statusMap = { queued: 0, running: 0, success: 0, failed: 0 };
    for (const r of all) {
