...
exports.listRollbackPoints = listRollbackPoints;
exports.getRollbackPointById = getRollbackPointById;
exports.createRollbackPoint = createRollbackPoint;
exports.executeRollback = executeRollback;
exports.getRollbackHistory = getRollbackHistory;
exports.retryRollback = retryRollback;
...
// ── Schemas ──────────────────────────────────────────────────────────────────
const createRollbackPointSchema = zod_1.z.object({
    deployment_id: zod_1.z.string().min(1, 'deployment_id is required'),
    from_revision_id: zod_1.z.string().min(1, 'from_revision_id is required'),
    to_revision_id: zod_1.z.string().min(1, 'to_revision_id is required'),
    reason: zod_1.z.string().default(''),
});
// ── List ─────────────────────────────────────────────────────────────────────
async function listRollbackPoints(query) {
...
// ── Get One ──────────────────────────────────────────────────────────────────
async function getRollbackPointById(id) {
    const dbInstance = (0, builtin_sqlite_js_1.getDatabase)();
...
async function createRollbackPoint(body) {
    const dbInstance = (0, builtin_sqlite_js_1.getDatabase)();
...
async function executeRollback(id) {
    const dbInstance = (0, builtin_sqlite_js_1.getDatabase)();
    const rp = dbInstance.prepare('SELECT * FROM rollback_points WHERE id = ?').get(id);
    if (!rp) {
    ...
    const toRev = dbInstance.prepare('SELECT * FROM deployment_revisions WHERE id = ?').get(rp.to_revision_id);
    if (!toRev) {
  ...
  `).run(nowStr, rp.deployment_id);
    ...
    return {
        ...
        message: `Rolled back from revision ${toRev.revision_number} to ${toRev.revision_number}`,
    };
}
...
// ── Retry Rollback ────────────────────────────────────────────────────────────
async function retryRollback(id) {
    const dbInstance = (0, builtin_sqlite_js_1.getDatabase)();
    const lastFailure = dbInstance.prepare(`SELECT * FROM audit_logs WHERE category = 'rollback' AND action = 'execute' AND target LIKE ? AND result = 'failed' ORDER BY created_at DESC LIMIT 1`).get(`%point:${id}%`);
