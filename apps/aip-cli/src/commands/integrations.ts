...
export async function runIntegrations(sub?: string) {
  ...
  if (sub === 'list') { for (const it of load()) console.log(`- ${it.name} | kind=${it.kind} | lifecycle=${it.lifecycle} | permission=${it.permissionLevel}`); } else { console.log('- planned subcommand: aip integrations list (implemented readonly)'); }
}
