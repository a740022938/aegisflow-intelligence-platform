...
export async function runProviders(sub?: string) {
  ...
  console.log(`Registry count: ${entries.length} providers (openAipv8CenterData.ts mirrors this data)`);
  if (sub === 'list') { for (const it of load()) console.log(`- ${it.name} | kind=${it.kind} | lifecycle=${it.lifecycle} | permission=${it.permissionLevel}`); console.log(`Total: ${entries.length}`); } else { console.log('- planned subcommand: aip providers list (implemented readonly)'); }
}
